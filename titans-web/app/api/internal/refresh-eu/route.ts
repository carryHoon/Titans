...
import { refreshEuStats } from '@/lib/eu-snapshot'

export const runtime        = 'nodejs'
export const dynamic        = 'force-dynamic'
export const maxDuration    = 300  // Vercel 상한. gap 6s면 65종목 ≈ 6.5분이라 이 라우트(서버리스)는
                                   // 완주 못 함 — 프로덕션 갱신은 GH Actions(무제한). 이 라우트는 로컬/수동 편의용.

// Euronext 시총 스냅샷 갱신 트리거(내부용, 로컬·수동 편의).
// ⚠️ 프로덕션 정기 갱신은 GitHub Actions(.github/workflows/refresh-eu.yml)가 러너에서
//    scripts/refresh-eu-stats.ts 를 직접 실행해 Upstash에 write 한다(refresh-us/jpx 와 동일 이유).
//
...
async function handle(req: Request): Promise<Response> {
  ...
  try {
    const { updated, failed } = await refreshEuStats()
    return NextResponse.json({ ok: true, updated, failed })
  } catch (err) {
    console.error('[refresh-eu] failed:', err)
