...
import { refreshNseStats } from '@/lib/nse-snapshot'

export const runtime        = 'nodejs'
export const dynamic        = 'force-dynamic'
export const maxDuration    = 300  // Vercel 상한. gap 6s면 30종목 ≈ 3분이라 이 라우트도 완주 가능하나,
                                   // 프로덕션 갱신은 GH Actions가 담당. 이 라우트는 로컬/수동 편의용.

// 인도 NSE 시총 스냅샷 갱신 트리거(내부용, 로컬·수동 편의).
// ⚠️ 프로덕션 정기 갱신은 GitHub Actions(.github/workflows/refresh-nse.yml)가 러너에서
//    scripts/refresh-nse-stats.ts 를 직접 실행해 Upstash에 write 한다(refresh-us/eu/cn 와 동일).
//
...
async function handle(req: Request): Promise<Response> {
  ...
  try {
    const { updated, failed } = await refreshNseStats()
    return NextResponse.json({ ok: true, updated, failed })
  } catch (err) {
    console.error('[refresh-nse] failed:', err)
